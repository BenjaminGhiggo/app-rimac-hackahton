/**
 * Treatments & Medication Context
 * Information about medication management, treatment adherence, and pharmaceutical services
 */

export const treatmentsContext = `
# RIMAC Medication & Treatment Management

## Medication Adherence

### What Is Adherence?
Taking medications exactly as prescribed by your healthcare provider - right dose, right time, right duration.

### Why It Matters
- Better treatment outcomes
- Prevents disease progression
- Reduces complications
- Avoids hospital readmissions
- Cost-effective healthcare
- Improves quality of life

### Adherence Risk Levels

#### 🟢 Low Risk (80-100% adherence)
- Excellent medication compliance
- Consistent refill patterns
- Regular follow-up with healthcare team
- **Action:** Continue current routine

#### 🟡 Medium Risk (60-79% adherence)
- Occasional missed doses
- Some delay in refills
- May benefit from reminders
- **Action:** Set up medication reminders, identify barriers

#### 🔴 High Risk (<60% adherence)
- Frequent missed doses
- Irregular refill patterns
- Multiple medications missed
- **Action:** Immediate intervention needed, healthcare team contact

### Common Barriers to Adherence
- Forgetting to take medication
- Complex medication regimens
- Side effects
- Cost concerns
- Not understanding importance
- Difficulty opening containers
- Running out of medication
- Feeling better and stopping early

### Solutions
- Medication reminders via app
- Pill organizers
- Simplified regimens when possible
- Address side effects with doctor
- Financial assistance programs
- Clear instructions
- Easy-open packaging
- Automatic refills

## Medication Management

### Medication Information
Each medication record includes:
- Name (generic and brand)
- Dosage and strength
- Frequency (how often)
- Route (oral, topical, injection, etc.)
- Purpose/indication
- Start and end dates
- Prescribing physician
- Refill information

### Medication Tracking Features
- Dose reminders
- Refill alerts
- Interaction warnings
- Side effect monitoring
- Progress tracking
- Adherence scores

### Common Medication Frequencies
- **QD (Once daily):** Take once per day
- **BID (Twice daily):** Take two times per day
- **TID (Three times daily):** Take three times per day
- **QID (Four times daily):** Take four times per day
- **PRN (As needed):** Take when symptoms occur
- **QHS (At bedtime):** Take before sleep

### Important Medication Instructions
- Take with or without food
- Avoid alcohol
- Don't crush or chew extended-release
- Store properly (temperature, light)
- Check expiration dates
- Don't share medications
- Complete full course of antibiotics

## Chronic Disease Medication Management

### Diabetes Medications
**Types:**
- Insulin (various types and durations)
- Metformin (first-line oral medication)
- Sulfonylureas
- DPP-4 inhibitors
- GLP-1 agonists
- SGLT2 inhibitors

**Adherence Importance:**
- Prevents blood sugar complications
- Reduces risk of heart, kidney, eye disease
- Maintains quality of life

**Monitoring:**
- Blood glucose testing
- A1C levels every 3 months
- Regular physician visits

### Hypertension Medications
**Types:**
- ACE inhibitors
- ARBs (Angiotensin receptor blockers)
- Calcium channel blockers
- Beta blockers
- Diuretics

**Adherence Importance:**
- Prevents stroke
- Reduces heart attack risk
- Protects kidneys
- Prevents heart failure

**Monitoring:**
- Home blood pressure monitoring
- Regular checkups
- Lab tests for kidney function

### Cholesterol Medications
**Types:**
- Statins (most common)
- Ezetimibe
- PCSK9 inhibitors
- Fibrates
- Bile acid sequestrants

**Adherence Importance:**
- Reduces cardiovascular disease risk
- Prevents heart attacks
- Reduces stroke risk

**Monitoring:**
- Lipid panel every 3-6 months
- Liver function tests
- Muscle pain assessment

### Asthma/COPD Medications
**Types:**
- Inhaled corticosteroids
- Long-acting bronchodilators
- Short-acting rescue inhalers
- Combination inhalers
- Leukotriene modifiers

**Adherence Importance:**
- Prevents exacerbations
- Maintains lung function
- Reduces emergency visits
- Improves daily activities

**Monitoring:**
- Peak flow measurements
- Symptom diary
- Inhaler technique review

### Mental Health Medications
**Types:**
- Antidepressants (SSRIs, SNRIs)
- Anti-anxiety medications
- Mood stabilizers
- Antipsychotics
- Sleep aids

**Adherence Importance:**
- Symptom control
- Prevents relapse
- Improves function
- Better quality of life

**Important Notes:**
- Don't stop suddenly (may cause withdrawal)
- Effects may take weeks
- Regular follow-up essential
- Report side effects immediately

## Medication Coverage

### Formulary Information
RIMAC covers medications in different tiers:

**Tier 1 (Generic):**
- Lowest copay
- Medically equivalent to brand
- Recommended first choice

**Tier 2 (Preferred Brand):**
- Moderate copay
- Brand-name medications
- Often have generic alternatives

**Tier 3 (Non-Preferred Brand):**
- Higher copay
- Specialty or newer medications
- Prior authorization may be required

**Tier 4 (Specialty):**
- Highest copay
- High-cost specialty drugs
- Prior authorization required
- May need specialty pharmacy

### Coverage by Plan
**Premium Plan:**
- 100% coverage for all tiers
- No copayments
- All pharmacies in network

**Plus Plan:**
- 70% coverage
- Copays vary by tier
- Network pharmacies preferred

**Basic Plan:**
- 50% coverage
- Higher copays
- Limited to network pharmacies

### Prior Authorization
Some medications require approval before coverage:
- Process initiated by physician
- Medical necessity documentation
- Response within 48-72 hours
- Alternative options if denied

## Pharmacy Services

### Partner Pharmacies
- Inkafarma
- Mifarma
- Universal
- Boticas y Salud
- Hospital pharmacies (network clinics)

### Pharmacy Services Available
- Prescription filling
- Medication counseling
- Drug interaction checks
- Generic substitution
- Medication therapy management
- Immunizations
- Health screenings

### Home Delivery
- Available for maintenance medications
- 90-day supply options
- Free delivery on most plans
- Automatic refills available
- Track delivery status

### Medication Reviews
Pharmacists can help with:
- Understanding medications
- Identifying interactions
- Optimizing therapy
- Cost-saving alternatives
- Proper medication use

## Medication Safety

### Drug Interactions
Be aware of interactions with:
- Other prescription medications
- Over-the-counter drugs
- Herbal supplements
- Vitamins and minerals
- Food and beverages (especially grapefruit, alcohol)

### Side Effects
**Common, usually mild:**
- Nausea
- Headache
- Drowsiness
- Dizziness

**Serious, require immediate attention:**
- Severe allergic reaction
- Difficulty breathing
- Chest pain
- Severe rash
- Unusual bleeding

### Medication Storage
- Keep in original containers
- Store at proper temperature
- Protect from light and moisture
- Keep out of reach of children
- Check expiration dates
- Dispose properly (don't flush)

## Treatment Plans

### Creating Treatment Plans
Your healthcare team develops plans that include:
- Medications prescribed
- Dosing schedule
- Expected outcomes
- Monitoring requirements
- Follow-up appointments
- Lifestyle modifications
- Warning signs to watch

### Monitoring Treatment Effectiveness
- Regular symptom assessment
- Lab work as needed
- Vital signs tracking
- Quality of life measures
- Functional improvement
- Adherence tracking

### Adjusting Treatment
Reasons for changes:
- Lack of effectiveness
- Side effects
- Better alternatives available
- Disease progression
- Cost considerations
- Patient preference

## Medication Assistance Programs

### Financial Assistance
- Manufacturer discount programs
- Patient assistance programs
- Generic alternatives
- Therapeutic substitutions
- Mail order savings

### Support Resources
- Medication counseling
- Adherence coaching
- Educational materials
- Support groups
- Mobile app reminders

## Gamification for Adherence

### Earning Points
- Taking medication on time
- Refilling prescriptions
- Attending follow-up appointments
- Completing medication reviews
- Achieving adherence goals

### Rewards
- Achievement badges
- Progress milestones
- Insurance premium discounts
- Gift cards
- Health product discounts

### Tracking Progress
- Daily adherence score
- Weekly/monthly trends
- Comparison to goals
- Streak tracking
- Community rankings (anonymous)
`;
